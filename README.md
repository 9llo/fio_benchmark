...
- **Full latency distribution** — separate read/write avg latency, stddev, p50, p99, and p99.9 per test
...
| # | Name | Pattern | Block Size | Jobs | iodepth | Size |
|---|------|---------|-----------|------|---------|------|
...
Random tests also use `--randrepeat=0` and `--norandommap` for unique access patterns and reduced overhead.
...
| RW-mix | `-m` | randrw, bs=4k, numjobs=8, iodepth=8 | write %: 0, 10, 20 … 100 |
...
```
=== FIO Benchmark v1.6.0 ===

  Running seqread                (bs=8k    jobs=8  iodepth=4   size=1G  ) ... Done.
  Running seqwrite               (bs=32k   jobs=4  iodepth=4   size=2G  ) ... Done.
  Running seq128kread            (bs=128k  jobs=8  iodepth=4   size=2G  ) ... Done.
  Running seq128kwrite           (bs=128k  jobs=4  iodepth=4   size=2G  ) ... Done.
  Running randread               (bs=8k    jobs=16 iodepth=8   size=1G  ) ... Done.
  Running randwrite              (bs=64k   jobs=8  iodepth=8   size=512m) ... Done.
  Running rand4kread             (bs=4k    jobs=16 iodepth=8   size=1G  ) ... Done.
  Running rand4kwrite            (bs=4k    jobs=8  iodepth=8   size=1G  ) ... Done.
  Running randrw                 (bs=16k   jobs=8  iodepth=8   size=1G  ) ... Done.
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------
Test                   | IOPS     | Bandwidth    | Rd Lat     | Wr Lat     | p50 Lat    | p99 Lat    | p99.9 Lat  | CPU (usr / sys)    | IOPS/CPU
-------------------------------------------------------------------------------------------------------------------------------------------------------------------
seqread                | 80300    | 627.3 MiB/s  | 0.09 ms    | 0.00 ms    | 0.07 ms    | 0.28 ms    | 0.54 ms    | 1.40% / 6.20%      | 9797
seqwrite               | 45641    | 1426.3 MiB/s | 0.00 ms    | 0.08 ms    | 0.07 ms    | 0.21 ms    | 0.41 ms    | 1.67% / 7.70%      | 4945
seq128kread            | 8010     | 1001.2 MiB/s | 0.12 ms    | 0.00 ms    | 0.11 ms    | 0.19 ms    | 0.31 ms    | 0.90% / 3.40%      | 1885
seq128kwrite           | 6821     | 852.6 MiB/s  | 0.00 ms    | 0.14 ms    | 0.13 ms    | 0.22 ms    | 0.38 ms    | 0.80% / 3.10%      | 1751
randread               | 83164    | 649.7 MiB/s  | 0.18 ms    | 0.00 ms    | 0.14 ms    | 0.42 ms    | 0.89 ms    | 0.55% / 2.93%      | 23780
randwrite              | 23423    | 1464.0 MiB/s | 0.00 ms    | 0.33 ms    | 0.28 ms    | 0.67 ms    | 1.18 ms    | 0.80% / 2.01%      | 8430
rand4kread             | 321000   | 1253.9 MiB/s | 0.39 ms    | 0.00 ms    | 0.34 ms    | 0.72 ms    | 1.44 ms    | 2.10% / 9.30%      | 28407
rand4kwrite            | 180500   | 705.1 MiB/s  | 0.00 ms    | 0.43 ms    | 0.38 ms    | 0.91 ms    | 1.83 ms    | 1.90% / 8.10%      | 18050
randrw                 | 52067    | 813.6 MiB/s  | 0.19 ms    | 0.28 ms    | 0.25 ms    | 0.43 ms    | 0.87 ms    | 1.16% / 4.77%      | 8787
-------------------------------------------------------------------------------------------------------------------------------------------------------------------
```
...
| **Rd/Wr Lat** | Per-direction average latency; Wr Lat = 0 on read-only tests, both are non-zero on mixed |
...
```json
{
  ...
  "runtime_secs": 60,
  "results": [
    {"test":"seqread","iops":80300,"bandwidth_mib":627.300,"r_lat_ms":0.0900,"w_lat_ms":0.0000,"r_std_ms":0.1000,"w_std_ms":0.0000,"lat_p50_ms":0.0700,"lat_p99_ms":0.2800,"lat_p999_ms":0.5400,"cpu_usr_pct":1.40,"cpu_sys_pct":6.20,"iops_per_cpu":"9797"},
    ...
  ],
  "iodepth_sweep": [
...
```
test,iops,bandwidth_mib,r_lat_ms,w_lat_ms,r_std_ms,w_std_ms,lat_p50_ms,lat_p99_ms,lat_p999_ms,cpu_usr_pct,cpu_sys_pct,iops_per_cpu
seqread,80300,627.300,0.0900,0.0000,0.1000,0.0000,0.0700,0.2800,0.5400,1.40,6.20,9797
seqwrite,45641,1426.300,0.0000,0.0800,0.0000,0.0900,0.0700,0.2100,0.4100,1.67,7.70,4945
...
```
